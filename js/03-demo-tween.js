...
function setupTween(){
    ...
    tween = new TWEEN.Tween(position) 
        .to({height:15}, 5000)                  //the finalv alue and the time to get there
        .delay(1000)                            //delay before starting tween
        .easing(TWEEN.Easing.Linear.None)       //how the values should change
        .onUpdate(updateHeight)

    let tween2 = new TWEEN.Tween(position)
        .to({height:0},2000)
        .easing(TWEEN.Easing.Exponential.ln)
        .onUpdate(updateHeight);
    tween.chain(tween2);
    tween2.chain(tween);
    tween.start();
...
function updateHeight(){
    cube.position.y = position['height'];
}
